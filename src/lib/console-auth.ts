...
import { isEnvironmentAdmin, type Scope } from './auth';
...
export function signedConsoleValue(payload: Record<string, unknown>, purpose: 'session' | 'oauth', env: NodeJS.ProcessEnv = process.env) {
...
export function readSignedConsoleValue(value: string, purpose: 'session' | 'oauth', env: NodeJS.ProcessEnv = process.env): Record<string, unknown> {
...
export function readConsoleSession(request: Request, env: NodeJS.ProcessEnv = process.env, now = Date.now()): Session {
  ...
  if (!Number.isSafeInteger(value.employeeId) || Number(value.employeeId) <= 0
    || typeof value.email !== 'string' || !/^[^\s@]+@wareongo\.com$/.test(value.email)
    || typeof value.sub !== 'string' || !/^[A-Za-z0-9_-]{1,255}$/.test(value.sub)
...
export async function resolveConsoleEmployee(client: PoolClient, email: string, expectedId?: number): Promise<ConsoleIdentity> {
  if (!/^[^\s@]+@wareongo\.com$/.test(email)) throw new HttpError(403, 'CONSOLE_ACCESS_DENIED', 'Use an active work account.');
  ...
  if (rows.length !== 1 || !employee || employee.is_active !== true || !Number.isSafeInteger(employee.id) || employee.id <= 0
    ...
    throw new HttpError(403, 'CONSOLE_ACCESS_DENIED', 'Use an active work account.');
  }
  const isAdmin = employee.adminAccess === true || isEnvironmentAdmin(email);
  const scopes: Scope[] = ['knowledge:read'];
  if (employee.dashboardAccess === true || isAdmin) scopes.push('warehouses:read');
  if (employee.twenty_user_id) scopes.push('crm:read');
  return { employeeId: employee.id, email, name: typeof employee.name === 'string' ? employee.name.slice(0, 200) : '', isAdmin, scopes };
}
...
export function consoleErrorResponse(error: unknown) {
  ...
  return consoleJson({ error: { code: safe.code, message: safe.message } }, safe.status);
}
