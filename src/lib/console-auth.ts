...
export const PASSWORD_SESSION_SUBJECT = 'password-admin';

export function consoleAdminEmail(env: NodeJS.ProcessEnv = process.env) {
  const email = (env.CONTEXT_ADMIN_EMAIL ?? '').trim().toLowerCase();
  if (email.length > 254 || !/^[^\s@]+@wareongo\.com$/.test(email)) {
...
export function signedConsoleValue(payload: Record<string, unknown>, purpose: 'session', env: NodeJS.ProcessEnv = process.env) {
...
export function readSignedConsoleValue(value: string, purpose: 'session', env: NodeJS.ProcessEnv = process.env): Record<string, unknown> {
  if (value.length > 4096 || !/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]{43}$/.test(value)) throw new HttpError(401, 'CONSOLE_UNAUTHENTICATED', 'Sign in to the admin console.');
  ...
  if (actual.length !== expected.length || !timingSafeEqual(expected, actual)) throw new HttpError(401, 'CONSOLE_UNAUTHENTICATED', 'Sign in to the admin console.');
  try {
    const decoded: unknown = JSON.parse(Buffer.from(body, 'base64url').toString('utf8'));
    if (!decoded || typeof decoded !== 'object' || Array.isArray(decoded)) throw new Error();
    return decoded as Record<string, unknown>;
  } catch { throw new HttpError(401, 'CONSOLE_UNAUTHENTICATED', 'Sign in to the admin console.'); }
}
...
export function readConsoleCookie(request: Request, kind: 'session' | 'oauth', env: NodeJS.ProcessEnv = process.env) {
  ...
  if (header.length > 16384) throw new HttpError(401, 'CONSOLE_UNAUTHENTICATED', 'Sign in to the admin console.');
  const entries = header.split(';').map(value => value.trim()).filter(value => value.startsWith(`${name}=`));
  if (entries.length !== 1) throw new HttpError(401, 'CONSOLE_UNAUTHENTICATED', 'Sign in to the admin console.');
...
export function readConsoleSession(request: Request, env: NodeJS.ProcessEnv = process.env, now = Date.now()): Session {
  ...
  if (!Number.isSafeInteger(value.employeeId) || Number(value.employeeId) <= 0
    || value.email !== consoleAdminEmail(env)
    || value.sub !== PASSWORD_SESSION_SUBJECT
    ...
    throw new HttpError(401, 'CONSOLE_UNAUTHENTICATED', 'Sign in to the admin console.');
  }
...
export async function resolveConsoleEmployee(client: PoolClient, email: string, expectedId?: number): Promise<ConsoleIdentity> {
  if (email !== consoleAdminEmail()) throw new HttpError(403, 'CONSOLE_ACCESS_DENIED', 'Console access is unavailable for this account.');
  ...
  // Console ownership permits knowledge editing, but does not widen source-data scopes.
  if (employee.dashboardAccess === true || employee.adminAccess === true) scopes.push('warehouses:read');
  if (employee.twenty_user_id) scopes.push('crm:read');
  return { employeeId: employee.id, email, name: typeof employee.name === 'string' ? employee.name.slice(0, 200) : '', isAdmin: true, scopes };
}
