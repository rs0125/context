...
import { getFreshness, getMyBriefing, getOpportunity, getWarehouse, getWarehouseFilterOptions, searchOpportunities, searchWarehouses, validateCrmQuery } from './data';
...
async function dispatch(client: PoolClient, principal: Principal, path: string[], query: URLSearchParams, crmAccess?: CrmAccess) {
  ...
  if (path[0] === 'crm' && (route === 'crm/opportunities' || route === 'crm/my-briefing'
    ...
    const view = route === 'crm/opportunities' ? validateCrmQuery(query).view : 'accessible';
...
export async function handleApiRequest(request: Request, path: string[], dependencies: Partial<ApiDependencies> = {}) {
  ...
  try {
    ...
    if (path[0] === 'crm') {
      ...
      if (route === 'crm/opportunities') view = validateCrmQuery(new URL(request.url).searchParams).view;
