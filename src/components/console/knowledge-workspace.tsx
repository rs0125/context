...
export function KnowledgeWorkspace({ writesEnabled, onSessionExpired, onDirtyChange, onBusyChange }: {
  ...
  return <div className={`knowledge-workspace ${styles.workspace}`}>
    <div className="page-heading"><div><p className="eyebrow">Knowledge base</p><h1>Company knowledge</h1><p>Write the guidance your team’s AI can refer to. Review a page when it’s ready to share.</p></div><div className="heading-actions"><button className="button button-secondary" disabled={saving} onClick={() => uploadRef.current?.click()}><Icon name="upload" />Import .md</button><button className={`button ${draft ? 'button-secondary' : 'button-primary'}`} disabled={saving} onClick={() => runOrConfirm(startNew)}><Icon name="plus" />New page</button></div></div>
    ...
    <div className="knowledge-grid">
      <aside className="panel knowledge-library" aria-label="Knowledge pages"><div className="library-heading"><h2>Pages <span aria-label={`${filtered.length} of ${pages.length} pages`}>{query ? `${filtered.length} / ${pages.length}` : pages.length}</span></h2><button className="icon-button" aria-label="Refresh page list" disabled={loading || saving} onClick={() => void loadPages()}><Icon name="refresh" size={16} /></button></div><div className="search-field"><Icon name="search" size={17} /><input aria-label="Search knowledge pages" placeholder="Find a page…" value={query} onChange={event => setQuery(event.target.value)} /></div>
        {loading ? <div className="library-state"><Spinner label="Loading pages…" /></div> : listError ? <div className="library-state"><Notice action={<button className="text-button" onClick={() => void loadPages()}>Retry</button>}>{listError}</Notice></div> : filtered.length ? <div className="page-list">{filtered.map(page => <button key={page.id} className={`page-list-item ${saved?.id === page.id ? 'selected' : ''}`} title={page.title} aria-current={saved?.id === page.id ? 'true' : undefined} disabled={saving} onClick={() => { if (saved?.id !== page.id) runOrConfirm(() => void openPage(page.id)); }}><span className="page-item-top"><Icon name="file" size={17} /><span className={`publication-badge publication-${page.status}`}><Icon name={page.status === 'reviewed' ? 'check' : 'file'} size={12} />{page.status === 'reviewed' ? 'Reviewed' : 'Draft'}</span></span><strong>{page.title}</strong><span className="page-item-summary">{page.summary}</span><span className="page-item-date">Updated {displayDate(page.updatedAt)}</span></button>)}</div> : <div className="library-state empty-library"><Icon name="book" size={27} /><h3>{query ? 'No matching pages' : 'A clean starting point'}</h3><p>{query ? 'Try another title or keyword.' : 'Add your first page or import a Markdown file.'}</p></div>}
      ...
      <section className="panel editor-panel" aria-label="Page editor" aria-busy={fetching || saving}>
        {fetching ? <div className="editor-loading"><Spinner label="Opening page…" /></div> : !draft ? <div className="editor-empty"><span className="editor-empty-icon"><Icon name="book" size={28} /></span><h2>Select a page to get started</h2><p>Open an existing page, or create a guide for your team’s agents.</p><button className="button button-secondary" onClick={() => runOrConfirm(startNew)}><Icon name="plus" />Create a page</button>{error && <Notice>{error}</Notice>}</div> : <>
          <div className="editor-toolbar"><div><span className={`publication-badge publication-${draft.status}`}><Icon name={draft.status === 'reviewed' ? 'check' : 'file'} size={12} />{draft.status === 'reviewed' ? 'Reviewed' : 'Draft'}</span><span className="editor-save-status">{dirty ? <><span className="unsaved-dot" />Unsaved changes</> : saved ? `Saved ${displayDate(saved.updatedAt)}` : 'New page'}</span></div><button className="button button-primary" aria-label={saving ? 'Saving…' : undefined} disabled={saving || !writesEnabled || (!dirty && Boolean(saved)) || conflict} onClick={() => void save()}>{saving ? <Spinner label="Saving…" /> : <><Icon name="check" size={17} />{publishing ? 'Save & publish' : draft.status === 'draft' ? 'Save draft' : 'Save changes'}</>}</button></div>
          <div className="editor-content">
            ...
            <fieldset className="editor-fields" disabled={saving}><legend className="sr-only">Page content and metadata</legend>
              ...
              <div className="field markdown-field"><div className="markdown-label"><label htmlFor="page-body">Markdown content</label><span id="markdown-size" className={bytes > 100_000 ? 'over-limit' : ''}>{(bytes / 1000).toFixed(1)} / 100 KB</span></div><textarea id="page-body" className="markdown-editor" aria-describedby="markdown-size" value={draft.body} onChange={event => updateDraft({ body: event.target.value })} placeholder={'# Start with what matters\n\nWrite the context an agent needs to give a useful answer.\n\n## Useful details\n\n- Be specific\n- Explain terms\n- Include what still needs verification'} spellCheck={false} /><div className="markdown-footer"><span>Plain Markdown · no rendered scripts</span><span>Stored privately · shared by permission</span></div></div>
              <div className="publication-settings"><div className="publication-heading"><Icon name="shield" size={18} /><h2>Sharing & review</h2></div>
