...
| --- | --- | --- |
| Discover permitted context | `get_context` | Returns the clock, scopes, query guidance and reviewed knowledge index. |
| Find company guidance | `search_knowledge(q="shortlist verification")`, then `read_knowledge(id=…)` | Search snippets are partial; read the relevant reviewed page and cite its date. |
...
| Build a specification shortlist | `search_warehouses(city="Bengaluru", docks_min=4, clear_height_min_ft=25)` | Default permissive matching admits estimates and overlapping ranges. Preserve uncertainty. |
...
- Search defaults to 10 records, with a maximum of 25. Keep filters and sort unchanged and pass `nextCursor` back unchanged. Date sorts use opaque cursors; default ID sorts retain legacy ID cursors. Do not treat the first page as a complete result or as the cheapest/best candidates.
...
Verification on 2026-09-26: the production build and automated tests passed; five warehouse integration checks and nineteen synthetic CRM PostgreSQL checks passed using bounded read-only transactions. The authenticated local MCP client completed the date searches, summaries, filter discovery, OAuth/PKCE and refresh-revocation checks. Eight natural-language model scenarios passed across 22 model calls using the live MCP definitions and synthetic results. That includes additions today in Bangalore, native monthly CRM creation, company lookup, full stage counts, tomorrow's follow-ups, contact/write refusal and uncertain specifications. This is a bounded smoke evaluation, not a guarantee of every model response. Remaining cases below are an evaluation backlog unless covered by deterministic tests. Private reports remain under `.local/tooling-eval/`.
