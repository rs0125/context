...
> Archived direction. The active interface follows [the Linear reference](linear-style-reference.md), selected subsequently.
...
- **Role:** Single-family system carrying all UI text. The two-weight binary (regular/bold) replaces color or size variation for hierarchy. Weight 400 reads as body and supporting text; weight 700 is reserved for emphasis lines, key headings, and navigation.
...
## Wareongo Context implementation notes

The reference above is preserved as supplied. Apply its visual system to Wareongo Context while retaining Wareongo's name and task-specific language for signing in, connecting an agent, managing an employee key, and editing knowledge. This is an application interface, so landing-page examples illustrate visual treatment rather than required page content.

The application self-hosts Inter through `src/app/fonts/InterVariable.woff2`, loaded with `next/font/local`; its SIL Open Font License is retained alongside the font. Inter is the reference's suggested substitute; the proprietary ABCfavorit Book font is not bundled. Use `-0.02em` display tracking as specified in the prose; the supplied table and quick-start snippets retain their original `-0.02px` values for fidelity. Keep essential text and controls readable: use Ink or Dim on light surfaces, reserving Ash for nonessential detail or on dark backgrounds.
